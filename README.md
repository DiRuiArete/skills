...
方法论这种东西，看的时候都懂，真到做决策的时候一条都想不起来。于是我试过把课程笔记直接喂给 AI 当资料库，结果它只会大段复述道理，该问我的问题一个不问。

所以我反过来做：让方法论来**问我**。呼出之后它先问你在哪个阶段、卡在哪，然后带你走对应的决策流。每个决策点都会提供选项，标一个推荐，打一个完整度分，并说一句理由，然后停下来等你拍板选择或你自己补充。查得到的事实它自己查，需要拿主意的事永远是你的。聊完后交付的是能直接用的东西，如：定位文档、复盘方案、增长作战地图，而不是一堆感悟。

交互上有几个讲究：不呼出就完全不存在，不会在无关对话里蹦出来；且一次只问一个问题；先问透了情况、达成了共识才动手；内容按需加载。这套交互范式借鉴了 mattpocock 的 grill-me 和 gstack 的 office-hours。
